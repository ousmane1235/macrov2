...
export default function MarketSessionsCard() {
  ...
  const parisTime = new Date().toLocaleTimeString("fr-FR", { timeZone: "Europe/Paris", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false });
  const activeSessions = SESSIONS.filter(s => isActive(s.openUTC, s.closeUTC));
  ...
  return (
    <div style={{ background: "#10101e", border: "1px solid #1c1c38", borderRadius: 12, padding: 20, height: "100%" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {SESSIONS.map(s => {
          const active = isActive(s.openUTC, s.closeUTC);
      ...
      {activeSessions.length >= 2 && (
        <div style={{ marginTop: 12, padding: "8px 12px", background: "rgba(212,175,55,0.06)", border: "1px solid rgba(212,175,55,0.2)", borderRadius: 8, fontSize: 12, color: "#f0c84a", fontWeight: 600 }}>
          ⚡ Overlap: {activeSessions.map(s => s.name).join(" + ")} — Haute volatilité
        </div>
      )}
    </div>
  );
