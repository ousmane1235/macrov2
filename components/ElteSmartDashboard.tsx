...
function computeDash(candles: Candle[], ewmaPeriod = 10, annual = 365, malen = 55, atrLen = 10): DashResult | null {
  ...
  const lH   = isNaN(hv[last])   ? 0 : hv[last];
  const lAvg = isNaN(avgHV[last]) ? 0 : avgHV[last];

  // Volume : bull vol vs bear vol sur les 20 dernières bougies complètes
  // On exclut la dernière (partielle) → slice jusqu'à ref inclus
  const volWindow = candles.slice(Math.max(0, ref - 19), ref + 1); // 20 barres max
  ...
  // Momentum : histogram MACD positif OU en hausse (condition inclusive)
  const momBull = hist[ref] > 0 || hist[ref] > hist[Math.max(ref-1, 0)];
